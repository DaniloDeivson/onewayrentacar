-- ============================================================================
-- CORREÇÃO FINAL DA FUNÇÃO DE VALIDAÇÃO DE QUILOMETRAGEM
-- ============================================================================
-- Esta migração corrige definitivamente o erro na função fn_validate_inspection_mileage

-- Atualizar a função de validação de quilometragem para usar o campo correto
...
BEGIN
  ...
  -- Obter quilometragem atual do veículo (usando o campo correto 'mileage')
  ...
  IF NEW.mileage < v_current_vehicle_mileage THEN
    ...
    IF v_mileage_difference <= 0.20 THEN
      -- Removido o RAISE NOTICE problemático - apenas permitir a correção
      RETURN NEW;
    ELSE
...
DO $$
BEGIN
  RAISE NOTICE 'Função fn_validate_inspection_mileage corrigida com sucesso!';
  RAISE NOTICE 'Agora usa o campo correto "mileage" em vez de "total_mileage"';
  RAISE NOTICE 'Erro de RAISE NOTICE removido completamente';
END $$; 
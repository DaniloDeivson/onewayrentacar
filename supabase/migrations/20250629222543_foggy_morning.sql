/*
  # Make costs immutable and add manager role

  1. Changes
    - Add permissions column to employees table if not exists
    - Add manager role to employees role check constraint
    - Create function to prevent cost deletion
    - Create function to prevent cost updates except for admin
    - Create triggers to enforce immutability
    - Add status 'Autorizado' to costs status check constraint

  2. Security
    - Only admin can modify or delete costs
    - Manager role has limited permissions
    - All cost entries are immutable once created
*/
...
BEGIN
  ...
  ELSE
    ...
    IF OLD.status = 'Pendente' AND (NEW.status = 'Pago' OR NEW.status = 'Autorizado') AND
       ...
       OLD.document_ref = NEW.document_ref AND
       OLD.observations = NEW.observations AND
       OLD.origin = NEW.origin AND
       OLD.created_by_employee_id = NEW.created_by_employee_id AND
       OLD.source_reference_id = NEW.source_reference_id AND
       OLD.source_reference_type = NEW.source_reference_type THEN

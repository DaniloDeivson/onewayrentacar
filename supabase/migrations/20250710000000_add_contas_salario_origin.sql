-- Add 'Contas' and 'salario' as valid origin values for costs
-- This migration updates the origin check constraint to include the new values for accounts payable integration
...
-- Add the new constraint with 'Contas' and 'salario' included
ALTER TABLE costs ADD CONSTRAINT costs_origin_check 
  CHECK (origin = ANY (ARRAY['Usuario'::text, 'Patio'::text, 'Manutencao'::text, 'Sistema'::text, 'Compras'::text, 'Contas'::text, 'salario'::text]));

-- Update the view to handle the new origins
...
SELECT 
  ...
  CASE 
    ...
    WHEN c.origin = 'Contas' THEN 'Contas a Pagar'
    WHEN c.origin = 'salario' THEN 'Salário'
    ELSE c.origin

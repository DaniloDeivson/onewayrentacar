...
-- 7. Update RLS policies for costs table to include department-based access
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 9. Drop existing function before recreating it
DROP FUNCTION IF EXISTS fn_get_pending_damage_notifications();

-- Create function to get pending damage notifications
CREATE FUNCTION fn_get_pending_damage_notifications()
...
DROP FUNCTION IF EXISTS fn_mark_notification_sent(UUID);
CREATE FUNCTION fn_mark_notification_sent(p_notification_id UUID)
RETURNS VOID AS $$
BEGIN
...
DROP FUNCTION IF EXISTS fn_mark_notification_failed(UUID, TEXT);
CREATE FUNCTION fn_mark_notification_failed(p_notification_id UUID, p_error_message TEXT)
RETURNS VOID AS $$
BEGIN
  UPDATE damage_notifications
  SET 

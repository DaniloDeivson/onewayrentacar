/*
# Update Inspections and Costs Tables

1. New Columns
   - Added contract_id, customer_id, mileage, and fuel_level to inspections table
   - Added department, customer_id, customer_name, and contract_id to costs table
   - Added km_limit, price_per_excess_km, and price_per_liter to contracts table

2. Indexes
   - Created indexes for new columns for better query performance

3. Functions
   - Updated fn_auto_create_damage_cost to include customer and contract information
   - Created fn_handle_rental_checkout for automatic charges on contract return
   - Updated views to include new fields
*/
...
-- 7. Check if policy exists before creating it
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'costs' AND policyname = 'Allow department-based access to costs'
  ) THEN
    EXECUTE $policy$

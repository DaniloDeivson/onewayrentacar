/*
  # Rental Checkout Integration with Damage Costs

  1. Changes
    - Add contract_id and customer_id columns to inspections table
    - Add department, customer_id, customer_name, and contract_id columns to costs table
    - Create function to handle rental checkout process
    - Create trigger for rental checkout
    - Update damage cost creation to include customer and contract information

  2. Security
    - Update RLS policies for costs table to include department-based access
    - Maintain existing tenant-based security model
*/

-- 1. Add new columns to inspections table
ALTER TABLE inspections ADD COLUMN IF NOT EXISTS contract_id UUID REFERENCES contracts(id);
ALTER TABLE inspections ADD COLUMN IF NOT EXISTS customer_id UUID REFERENCES customers(id);
...
ALTER TABLE costs ADD COLUMN IF NOT EXISTS department TEXT;
ALTER TABLE costs ADD COLUMN IF NOT EXISTS customer_id UUID REFERENCES customers(id);
ALTER TABLE costs ADD COLUMN IF NOT EXISTS customer_name TEXT;
ALTER TABLE costs ADD COLUMN IF NOT EXISTS contract_id UUID REFERENCES contracts(id);
...
-- 3. Create function to handle rental checkout process
...
BEGIN
  ...
  IF v_checkout_inspection.id IS NOT NULL AND NEW.mileage IS NOT NULL AND v_checkout_inspection.mileage IS NOT NULL THEN
    ...
    IF v_contract.km_limit IS NOT NULL AND v_contract.price_per_excess_km IS NOT NULL THEN
  ...
  IF v_checkout_inspection.id IS NOT NULL AND 
    ...
    IF v_fuel_difference < 0 THEN
      ...
      v_fuel_charge := ABS(v_fuel_difference) * 100 * v_contract.price_per_liter;
    END IF;
  END IF;
  ...
  IF v_fuel_charge > 0 THEN
    INSERT INTO costs(
      ...
      'Combustível', 
      NEW.vehicle_id,
      CONCAT('Reabastecer ', ABS(v_fuel_difference) * 100, '% — Cliente: ', v_customer_name),
      v_fuel_charge, 
      NEW.inspected_at::date, 
      'Pendente',
      CONCAT('Contrato ', NEW.contract_id, ' - Nível inicial: ', v_fuel_level_start * 100, '%, Nível final: ', v_fuel_level_end * 100, '%'),
      'Sistema',
      now()
    );
  END IF;
...
-- 4. Update damage cost creation to include customer and contract information
...
-- 5. Update RLS policies for costs table to include department-based access
...
-- 6. Add columns to contracts table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 7. Add columns to inspections table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
